---
layout: default
title: Adding Message to ProPresenter
parent: Media
nav_order: 3
---

# Adding Message to ProPresenter
{: .no_toc }
This is for adding a new message for a service like Sunday morning or special service like Good Friday, Thanksgiving Eve, etc.
...
- If there are images, they should be placed within a folder for the current date (yyyy-mm-dd) within the Sanctuary_Service_Graphics folder
  - In Finder the folder should be on the sidebar, and can be found at /users/&lt;username&gt;/Sync/ProPresenter_Shared_Content/Sanctuary_Service_Graphics
  - Create a new folder for the date of the message (for example 2026-01-01)
...
- A Timer set to the correct time (using main timer and setting it to count down a certain amount or count down to a specific time, depending on the speaker’s preferences)
  - If a timer is used to count down to time you will need to add one per service
...
To add a slide in edit, click the plus symbol on the top left of the screen, choose the theme from “Message Series”, and then choose the slide you want to add. Once you have chosen the theme, it will pop up by default when you click the plus symbol.

## Adding a Point Slide
For a point slide, choose the Point LT slide. This will be for the lower thirds and then converted for displaying on the LED wall. Once the slide is added, copy and paste the point into the text box. After copying, use Command + Option + Shift + V to paste without formatting. This will allow you to use the formatting within the textbox. If you just paste and the formatting is messed up best to undo and try again or re-apply the theme by right clicking on the slide and choosing themes and then pick the correct theme slide.

When adding a point, if it will be more than one line you may need to manually add a line break to allow it to wrap nicely and look good on the lower thirds and the wall. Best to keep multiple lines the same ish width if possible or break in a logical space like a comma. On some occasions if the point is long might want to talk with the speaker about shortening it or put the longer point into the Scripture/Quote slide format and remove the reference (see below).

## Adding a Scripture/Quote Slide
Use the Scripture/Quote Wall slide for this as we are not using lower thirds for Scripture/Quotes at this time. Copy/paste the verse (or quote) and reference (or person who said the quote) into the slide. It is best to have the reference (or person) on the slide first. Once both are added go through the scripture (or quote) and remove verse numbers, reference markers, etc. that are not needed for displaying on the screen.

The next step is fitting the content, typically there are only four lines on the screen. Any more than that and it may be too long or hard to find your place if there is a pause. Sometimes another line is added to keep things together but often then it may be split into two slides. To split content into multiple slides, edit the text box with the content and look for places to make a break, usually at commas, periods, or pauses in the content. Add two line breaks to test it out and see how it will look for all slides. Once you know where the breaks will be, go to the break and press Option + Enter. This will move the content below the cursor to the next slide and copy the reference using the same theme slide. You can do this multiple times to copy all content onto as many slides as needed.

While fitting the content, you should check to make sure that things look alright and there aren't any stray words. If there is a single word on the last line, there are a couple things you can do to clean it up and make it look better. You can try increasing or decreasing the font size by a couple points to see if that fixes it. Sometimes it only needs one or two points up or down to pull that last word off its own line. You can also make the textbox a little bigger or smaller. Hold down Option when expanding the right or left to expand both ways at the same time. With either method, just make sure it looks good with all the other slides in the same section.

Once all are done, add a blank slide so the operator can know when they are at the end of the scripture or quote.
...
Switch to the edit view and if the image does not fill the screen it can be enlarged to fill the screen or a black box can be added behind the image to block out the background and show the image on the screen. Do this by adding a square shape, expand it to fill the entire screen, set the color to black (on right side in shape section), and then on the bottom left, under objects, move the box down so the image is above it.
...
# Testing
Click through all slides and test them to make sure they appear as they should on the wall, broadcast, and the stage display. Correct any issues as they arise and then test again.
